...
const OPTIONS = [
  { id: "instagram", labelKey: "instagram" as const, href: "#", icon: "ig" as const },
  { id: "messenger", labelKey: "messenger" as const, href: "#", icon: "fb" as const },
] as const;

export default function ContactFloat({ whatsappLink }: { whatsappLink?: string | null }) {
      ...
      >
        ...
        {OPTIONS.map((opt, i) => (
          <a
            ...
            className="contact-float-option flex items-center gap-3 rounded-full bg-white px-4 py-2.5 text-sm font-medium text-slate-700 shadow-lg ring-1 ring-slate-200/80 transition hover:bg-slate-50 hover:shadow-xl"
            style={
              open ? { animationDelay: `${i * 0.05}s` } : undefined
            }
          >
